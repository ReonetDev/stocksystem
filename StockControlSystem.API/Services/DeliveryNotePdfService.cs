    ...
    public class DeliveryNotePdfService
    {
        ...
        public DeliveryNotePdfService(ApplicationDbContext context)
        {
            ...
            else // We are in a production environment
            {
                ...
                {
                    // On macOS: assemblyLocation is .../Contents/Resources/api/osx-arm64/
                    // Navigate up to Resources directory (2 levels up), then into assets
                    var currentDir = new DirectoryInfo(assemblyLocation);
                    var apiDir = currentDir.Parent; // Go up from 'osx-arm64' to 'api'
                    var resourcesDir = apiDir!.Parent; // Go up from 'api' to 'Resources'
                    prodPath = Path.Combine(resourcesDir!.FullName, "assets");
                }
                else
                {
                    // Windows path
                    prodPath = Path.GetFullPath(Path.Combine(assemblyLocation, "..", "..", "assets", "assets"));
                }
        ...
        public byte[] GeneratePdf(int deliveryNoteId)
        {
            ...
            // Find the file that starts with "reologo" and ends with ".gif"
            var assetsDir = new DirectoryInfo(_assetsPath);
            var logoFile = assetsDir.GetFiles("reologo*.gif").FirstOrDefault();
                    ...
                    container.Page(page =>
                    {
                        ...
                        page.Header()
                            ...
                            .Row(row =>
                            {
                                ...
                                row.ConstantItem((float)87.04).Image(logoFile.FullName);
                            });
            ...
            {
                throw new FileNotFoundException($"Could not find reologo.gif in {_assetsPath}");
            }
        }
    }
}